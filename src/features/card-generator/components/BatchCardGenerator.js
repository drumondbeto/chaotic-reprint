...
import { generatePdfForDownload } from '../utils/pdfGenerator';
...
export class BatchCardGenerator {
  ...
  async getAllCardsBlob(cardList = [], locale = 'pt') {
    cardList = this.filterUnofficials(cardList);
    // const zip = new JSZip();
    const total = cardList.length;
    let completed = 0;
  ...
  async generatePdfFromCardList(cardList = [], pdfFilename = 'chaotic-cards.pdf', locale = 'pt') {
    var blobs = await this.getAllCardsBlob(cardList, locale);
    return await generatePdfForDownload(blobs, pdfFilename);
  }
